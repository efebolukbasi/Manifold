/**
 * Codex CLI Adapter for the desktop bridge.
 * Mirrors packages/cli/src/portal/codex-adapter.ts
 */
  ...
  async sendMessage(
    ...
    try {
      const result = await runCodexCommand(
        ["exec", "--full-auto", "--color", "never", "--ephemeral", "-C", this.options.cwd, "-o", outputFile, "-"],
        this.options.cwd,
        prompt
      );
      ...
      return {
        message: createMessage({ from: this.config.id, to: "user", type: "response", content }),
...
async function runCodexCommand(args: string[], cwd: string, stdin?: string): Promise<{ stdout: string; stderr: string; exitCode: number | null }> {
  return new Promise((resolve, reject) => {
    const child = spawn("codex", args, { cwd, shell: process.platform === "win32", windowsHide: true });
    let stdout = "", stderr = "";
